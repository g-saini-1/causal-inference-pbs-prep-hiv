...
rng = np.random.default_rng(20180401)
...
for state in STATES:
    ...
    for m in months:
        ...
        # NSW-only early EPIC-NSW ramp (small, trial-scale, pre-listing)
        if state == "NSW" and m < PBS_LISTING and months_since_epic >= 0:
            level += min(700, 40 * months_since_epic)
        ...
        if m >= PBS_LISTING:
            ...
            level += base_national_scale * w * ramp * growth
...
hiv.to_csv("/home/claude/synth/hiv_notifications_quarterly.csv", index=False)
...
covariates.to_csv("/home/claude/synth/state_covariates.csv", index=False)

print("\nFiles written:")
for f in ["pbs_dispensing_monthly.csv", "hiv_notifications_quarterly.csv", "state_covariates.csv"]:
    df = pd.read_csv(f"/home/claude/synth/{f}")
